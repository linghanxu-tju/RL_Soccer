...
def sac(env_fn, actor_critic=MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    for t in range(total_steps):
        ...
        # CPC update handing
        if args.cpc and e > args.cpc_batch * 2 and t % args.cpc_update_freq == 0:
            for _ in range(args.cpc_update_num):
                if _ == 0:
                    update_cpc(True)
                else:
                    update_cpc(False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cpc_update_num', type=int, default=1, )
