...
def sac(env_fn, actor_critic=MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    bufferopp1 = ReplayBuffer(obs_dim = obs_dim, size=replay_size)
    bufferopp3 = ReplayBuffer(obs_dim = obs_dim, size=replay_size)
    ...
    for t in range(total_steps):
        ...
        if t >= update_after and t % update_every == 0:
            for j in range(update_every):
                batch1 = bufferopp1.sample_batch(int(batch_size*args.opp1_per), device=torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu"))
                batch3 = bufferopp3.sample_batch(int(batch_size*args.opp3_per), device=torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--opp1_per', type=float, default=1.0)
    parser.add_argument('--opp3_per', type=float, default=0.0)
    parser.add_argument('--opp_freq', type=int, default=1)

    args = parser.parse_args()

    save_dir = os.path.join(args.save_dir, str(args.opp1_per) + '_' + str(args.opp3_per))
    ...
    sac(lambda: SoccerPLUS(visual=False), actor_critic=MLPActorCritic,
        ...
        gamma=args.gamma, seed=args.seed, epochs=args.epochs, replay_size=args.replay_size,
        lr=args.lr, alpha=args.alpha, batch_size=args.batch_size, start_steps=10000, steps_per_epoch=1000, polyak=0.995,
        update_after=10000, update_every=1, num_test_episodes=100, max_ep_len=1000, save_freq=200,
